  ...
  function stopRecording() { endMeeting(); }
  ...
  async function endMeeting() {
    ...
    document.getElementById('notesMeta').textContent = `${dateStr} · ${timeStr}`;
    document.getElementById('notesDuration').textContent = `Duration: ${formatTime(secondsElapsed)}`;
    ...
    document.getElementById('notesBullets').innerHTML = '<li>Extracting key points…</li>';
    ...
    try {
      ...
      document.getElementById('notesBullets').innerHTML = data.bullets.map(b => `<li>${b}</li>`).join('');
      ...
      document.getElementById('notesSummary').textContent = 'Could not connect to backend. Check that your server is running on port 8000.';
  ...
  function copyNotes() {
    const text = [
      ...
      '',
      'KEY POINTS',
      ...Array.from(document.querySelectorAll('#notesBullets li')).map(li => '• ' + li.textContent),
      '',
      'FULL TRANSCRIPT',
    ...
    navigator.clipboard.writeText(text).then(() => {
      ...
      setTimeout(() => { btn.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> Copy`; }, 2000);
    });
  }
...
async function downloadPDF() {
  ...
  function newMeeting() {
    ...
    summaryData = null;
    bulletsData = [];
    document.getElementById('liveText').innerHTML = '<div class="placeholder">Listening…</div>';
    document.getElementById('liveSummary').className = 'panel-body placeholder';
    document.getElementById('liveSummary').textContent = 'Summary will appear after recording ends';
    document.getElementById('liveBullets').className = 'panel-body placeholder';
    document.getElementById('liveBullets').textContent = 'Key points will be extracted from your meeting';
